    ...
    def radix_sort(a):
        ...
        if len(a) > 0:
            ...
            while temp is not None:
                ...
                temp._next = None
                bucket[temp._value % 10].append(temp)
                temp = a._front
            odd_bucket = True
            if digit > 1:
                i = 1
                while i < digit:
                    if odd_bucket:
                        ...
                        while j < len(bucket):
                            k = 0
                            while k < len(bucket[j]):
                                bucket2[bucket[j][k]._value // 10**i %
                                        10].append(bucket[j][k])
                                bucket[j].pop(k)
                            j += 1
                    else:
                        j = 0
                        while j < len(bucket2):
                            k = 0
                            while k < len(bucket2[j]):
                                bucket[bucket2[j][k]._value // 10**i %
                                       10].append(bucket2[j][k])
                                bucket2[j].pop(k)
                            j += 1
                    odd_bucket = not odd_bucket
                    i += 1
            previous = None
            if odd_bucket:
                for v in bucket[::-1]:
